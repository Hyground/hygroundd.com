...
const previewRoutes: Record<string, string> = { "pdf-word": "/tools/pdf-to-word", "word-pdf": "/tools/word-to-pdf", "im-genes-pdf": "/tools/images-to-pdf", "pdf-im-genes": "/tools/pdf-to-images", "comprimir-pdf": "/tools/compress-pdf", "comprimir-im-genes": "/tools/compress-images", "jpg-png": "/tools/jpg-to-png", "png-jpg": "/tools/png-to-jpg", "contador-de-palabras": "/tools/word-counter", "password-generator": "/tools/password-generator", "qr-generator": "/tools/qr-generator", "calculadora-de-porcentajes": "/tools/percentage-calculator" };
const make = (category: ToolCategory, name: string, icon: string, options: Partial<Omit<Tool, "id" | "slug" | "name" | "category" | "icon">> = {}): Tool => { const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""); const previewRoute = previewRoutes[slug]; return { id: category+"-"+slug, slug, name, shortDescription: options.shortDescription ?? "Herramienta online gratuita.", description: options.description ?? "Herramienta online gratuita para tareas diarias.", category, icon, route: options.route ?? previewRoute ?? null, status: options.status ?? (previewRoute ? "beta" : "coming-soon"), featured: options.featured ?? false, popularityRank: options.popularityRank, keywords: options.keywords ?? [name, category] }; };
